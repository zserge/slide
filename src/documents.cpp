...
Document_b::Document_b(const int w, const int h, cairo_surface_t * const psurf, cairo_t * const pcr, const std::string& name) : Page_b(w, h, psurf, pcr, name)
{/* intentionally blank */}

Document_b::Document_b(const int w, const int h, const std::string& name): Page_b(w, h, name)
{/* intentionally blank */}

Page_b::Page_b(const int w, const int h, cairo_surface_t * const psurf, cairo_t * const pcr, const std::string& name )
		: _size(w,h), _surface(psurf), _cr(pcr), _name(name)
{/* intentionally blank */}

Page_b::Page_b(void) : _size(0, 0), _surface(0), _cr(0), _name("No Name Set")
{/* intentionally blank */}

PNG::PNG(const int w, const int h) : Page_b(w, h, "No Name Set")
{ /* Intentionally Blank */ }

PNG::PNG(const int w, const int h, const std::string& name) : Page_b(w, h, name)
{ /* Intentionally Blank */ }
...
void PNG::text(const std::string &text, const colour_t& color, int x, int y, slide::Style style, float scale) {
...
PDF::PDF(const std::string name, const int w, const int h) : Document_b(w, h, name)
{ /* Intentionally Blank */ }
...
void PDF::end_page(void) {
	wcairo::show_page(_cr);
}
...
void PDF::text(const std::string &text, const colour_t& color, int x, int y, slide::Style style, float scale) {
	wcairo::text(_cr, text, color, x, y, style, scale);
}