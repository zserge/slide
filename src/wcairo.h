...
class wcairo {
public:
	static cairo_surface_t* create_surface(int width, int height, cairo_format_t format = CAIRO_FORMAT_ARGB32);
	...
	static int 	text_width(cairo_t *cr, const std::string &text, slide::Style style, float scale);
	static void bg(cairo_t *cr, int w, int h, const colour_t& color);
	static void text(cairo_t *cr, const std::string &text, const colour_t& color, int x, int y, slide::Style style, float scale);
	...
	static void write_to_png_stream (cairo_surface_t *surface, cairo_write_func_t write_func, void *closure);
};
